...
double calculate_pi_neon(long long N) {
    ...
    const float64x2_t one_vec = vdupq_n_f64(1.0);
    ...
    if (N % 2 != 0) {
        double last_den = (double)(N - 1) * 2.0 + 1.0;
        double last_term = ((N - 1) % 2 == 0) ? (1.0 / last_den) : (-1.0 / last_den);
        sum += last_term;
    }
    return sum * 4.0;
}
...
double calculate_pi_sve(long long N) {
    ...
    long long i = 0;
    svbool_t pg_all = svptrue_b64(); // Predicate true for all lanes (used for final reduction and unpredicated add)
    ...
    while (i < N) {
        // Determine active lanes predicate for this iteration
        svbool_t pg_iter = svwhilelt_b64((int64_t)i, (int64_t)N);

        // Generate indices for active lanes, others zero
        svuint64_t indices = svindex_u64(0ULL, 1ULL); // Generate 0, 1, 2...
        indices = svadd_u64_z(pg_iter, indices, svdup_n_u64(i)); // indices = i, i+1,... (zeroing inactive)
...
int main(int argc, char *argv[]) {
    ...
    if (target_core >= 0) {
        ...
        if (getenv("GITHUB_ACTIONS") != NULL) {
            ...
            if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset) == -1) {
                ...
                fprintf(stderr, "Warning: Could not set CPU affinity to core %d. Running on any allowed core.\n", target_core);
                target_core = -1; // Indicate failure
            } else {
                printf("Successfully set CPU affinity to core %d for the duration.\n", target_core);
            }
        }
    } else {
    ...
    // Decide execution order (e.g., NEON first, SVE second for consistency with V4)
    // You can swap these blocks if needed
    ...
    long long duration_neon = end_time_neon - start_time_neon;
    ...
    if (isnan(pi_sve_res)) {
        ...
        duration_sve = end_time_sve - start_time_sve;
    ...
    if (duration_sve >= 0) { // Only print SVE duration if calculation succeeded
        ...
        if (duration_neon > 0 && duration_sve > 0) {
            ...
            double percentage = (diff / (double)duration_neon) * 100.0;
            ...
            if (diff > 0) {
             ...
             printf("Ratio (NEON time / SVE time): %.3f\n", (double)duration_neon / duration_sve);
        }
    } else {
        printf("SVE Calculation failed, comparison skipped.\n");
    }
    printf("Target Core: %s\n", (target_core >= 0 && getenv("GITHUB_ACTIONS") == NULL) ? argv[2] : "Any/Not Set"); // Adjusted target core output
